...
selected_columns <- grep("SNP_ID|Chromosome|Position|ZMMIL|ZMMLR|ZMMMR", names(merged_data), value = TRUE) #Pulled the columns that contains SNP_ID, Chromosome, Position, ZMMIL, ZMMLR, ZMMMR from the merged_data file to merged_data_subset file
merged_data_subset <- merged_data[, selected_columns] ##Pulled the columns that contains SNP_ID, Chromosome, Position, ZMMIL, ZMMLR, ZMMMR from the merged_data file to merged_data_subset file
...
write.table(rbind(colnames(merged_data_subset), merged_data_subset[merged_data_subset[, 2] == unknown, ]), file = "maize_chr_unknown.txt", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t") #code used to sort data according to chromosome numbers from 1-10 from merged_data_subset to respective chromosome files; this code sorts files directly into the working directory
maize_chr3 <- read.table("maize_chr3.txt", header = TRUE, sep = "\t") #this code was used for all respective chromosome files to load them from the working directory into the dataframes
